import { WeatherUtils } from "./weather.types.js";
import { API_CONFIG, WEATHER_CODES, APP_CONSTANTS, } from "../config/constants.js";
...
export class WeatherService {
    static async getCurrentWeather(lat, lng) {
        ...
        return await RetryHelper.retryApiCall(async () => {
            ...
            const data = await APIHelper.fetchData(url, "Failed to get weather data");
            APIHelper.validateData(data, (d) => Boolean(d.current), "No current weather data available");
            return {
    ...
    static async getWeatherWithFallback(lat, lng, fallbackData = null) {
        try {
            ...
            Logger.warn(`Weather API failed, using fallback data for ${lat}, ${lng}`, error);
            if (fallbackData && this.isWeatherDataFresh(fallbackData)) {
                Logger.info("Using fresh cached weather data");
                return fallbackData;
            }
            else if (fallbackData) {
                Logger.warn("Cached weather data is stale but will be used as fallback");
